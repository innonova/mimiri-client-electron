	...
	try {
		...
		if (process.platform === 'linux') {
			...
			writeFileSync('./artifacts.json', JSON.stringify([
				`dist-bin/mimiri-notes_${packageJson.version}_${arch}.tar.gz`,
				`dist-bin/mimiri-notes_${packageJson.version}_${arch}.snap`,
				`dist-bin/mimiri-notes_${packageJson.version}_${arch}.AppImage`,
				`dist-bin/io.mimiri.notes_${packageJson.version}_${arch}.flatpak`,
			], undefined, '  '));
		}
		...
		if (process.platform === 'win32') {
			...
			const releasePath = Path.join(electronWinInstallerPath, `electron-win.${packageJson.version}.json`);
		...
		if (process.platform === 'darwin') {
			...
			const releasePath = Path.join(electronWinInstallerPath, `electron-darwin.${packageJson.version}.json`);
		...
		console.error('Error renaming packages:', error);
		process.exit(1);
	}
};
