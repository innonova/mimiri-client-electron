...
const execute = async () => {
	try {
		...
		const electronSource = doc.modules
			...
			.sources.find(s => s['url'] === 'https://github.com/innonova/mimiri-client-electron');
		...
		const bundleSource = doc.modules
			.find(m => m.name === 'mimiri-notes')
			.sources.find(s => s['dest-filename'] === 'bundle.json');
		...
		console.error('Error updating flatpak:', error);
		process.exit(1);
	}
};
