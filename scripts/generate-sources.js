...
const fileEntries = []
const inlineEntries = []
const electronArchitectures = [{ name: 'arm64', arch: 'aarch64' }, { name: 'armv7l', arch: 'arm' }, { name: 'x64', arch: 'x86_64' }]
...
const main = async () => {
	...
	fileEntries.push(await createElectronFileEntry(`https://github.com/electron/electron/releases/download/v${electronVersion}/SHASUMS256.txt`, `SHASUMS256.txt-${electronVersion}`))
	for (const cpuArch of electronArchitectures) {
		fileEntries.push(await createElectronFileEntry(`https://github.com/electron/electron/releases/download/v${electronVersion}/electron-v${electronVersion}-linux-${cpuArch.name}.zip`, `electron-v${electronVersion}-linux-${cpuArch.name}.zip`))
	}
